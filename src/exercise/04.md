...
obs: Inline style properties are written in camelCase. For example,
```html
...
Font: https://react.dev/learn/javascript-in-jsx-with-curly-braces

### propTypes feature
The goal of the propTypes feature is to make sure you are passing all the props as expected to the component. So you get if all pre validated.
Note that prop types validation add some runtime overhead resulting in sub-optimal performance, so the validation functions are not run in production.

### **React Fragments**

It's a special kind of component from React which allows you to position two elements side-by-side rather than just nested.

The component is available via `<React.Fragment>` (or a [short syntax](https://react.dev/reference/react/Fragment#returning-multiple-elements) that opens with `<>` and closes with `</>`).
